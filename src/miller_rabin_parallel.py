...
def _mr_worker(args):
    n, a, d, r = args
    return _miller_rabin_test(n, a, d, r)
...
def is_prime(n: int) -> bool:
    ...
    witnesses = [random.randint(2, n - 2) for _ in range(DEFAULT_ROUNDS)]
    tasks = [(n, a, d, r) for a in witnesses]

    with Pool(cpu_count()) as pool:
        results = pool.map(_mr_worker, tasks)
